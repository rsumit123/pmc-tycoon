...
- **`AcquisitionPipeline` filters out platforms with any active order** in the Offers section (`frontend/src/components/procurement/AcquisitionPipeline.tsx`). This prevents multi-batch procurement of the same platform (e.g. can't sign "Tejas Mk1A batch 2" while batch 1 is still being delivered), which is a real-world pattern. Remove the filter and let the POST `/acquisitions` endpoint allow repeat orders. Landed mid-implementation to resolve a test ambiguity. (Plan 7 → Plan 10)
- **`RDDashboard` funding-level buttons are icon-only** (`↓` / `●` / `↑`) instead of text labels ("slow"/"standard"/"accelerated"). Landed to avoid a test `getByText(/standard/i)` ambiguity. Icons-without-tooltips are less discoverable; add `title` attributes or text labels in Plan 10 UI polish. (Plan 7 → Plan 10)
