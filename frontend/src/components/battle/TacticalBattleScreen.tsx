...
export const TacticalBattleScreen = ({ battleId, initialState, onComplete }: TacticalBattleScreenProps) => {
  ...
  const handleAction = async (action: TacticalAction) => {
    ...
    try {
      const res = await apiService.submitChoice(battleId, action.key, action.weapon_id ?? undefined);
      ...
      if (data.enemy_shot_hit !== undefined && data.enemy_shot_hit !== null) {
        ...
        }, data.shot_hit !== undefined ? 2000 : 300);
      }
      ...
      if (data.zone !== state.zone) {
        addLog('NAV', 'text-amber-400', `Zone change: ${data.zone}`);
      }
  ...
  const handleNextTurn = async () => {
    ...
    // Already have next state from the turn result
    if (turnResult?.state) {
      setState(turnResult.state);
    }
    setTurnResult(null);
    ...
    <div className={`min-h-[100dvh] bg-gray-950 flex flex-col hud-grid hud-scanlines relative ${screenEffect === 'damage-vignette' ? 'screen-shake' : ''}`}>
      ...
      <div className="px-3 py-2 flex items-center justify-between border-b border-emerald-500/10">
        <div className="hud-text flex items-center gap-2">
          <span className="text-[9px] text-emerald-400/60">TURN</span>
          <span className="text-sm font-bold text-emerald-400 hud-glow">{state.turn}/{state.max_turns}</span>
          <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border ${zone.bg} ${zone.text}`}>
            {state.zone}
          </span>
        </div>
        ...
        <div className="hud-border rounded-xl p-3 relative overflow-hidden bg-gray-950/80">
          ...
          <div className="relative flex items-center justify-between mb-2">
            ...
            <div className="text-center">
              ...
              <p className="text-[8px] text-emerald-400 hud-text font-bold truncate max-w-[60px]">{state.player_name.split(' ').pop()}</p>
              <div className="h-1 w-12 bg-gray-800 rounded-full overflow-hidden mt-0.5">
                <div className={`h-full rounded-full ${playerHp > 60 ? 'bg-emerald-500' : playerHp > 30 ? 'bg-amber-500' : 'bg-red-500'}`} style={{ width: `${playerHp}%` }} />
              </div>
              <span className="text-[8px] text-gray-500 hud-text">{playerHp.toFixed(0)}%</span>
            </div>
            ...
            <div className="flex-1 mx-2 relative h-6 flex items-center">
              ...
              <div className="absolute top-full left-1/2 -translate-x-1/2 mt-0.5">
                <span className="text-[8px] text-gray-600 hud-text">{state.range_km.toFixed(0)}KM</span>
              </div>
            </div>
            ...
            <div className="text-center">
              <div className="w-12 h-12 rounded-lg hud-border-amber flex items-center justify-center bg-amber-500/5 mb-1 relative">
                <span className="text-xl">◇</span>
                {intelCount < 6 && (
                  <span className="absolute -top-1 -right-1 text-[8px] bg-violet-500/30 text-violet-300 rounded-full w-4 h-4 flex items-center justify-center font-bold">?</span>
                )}
              </div>
              <p className="text-[8px] text-red-400 hud-text font-bold truncate max-w-[60px]">{state.enemy_intel.name.split(' ').pop()}</p>
              {/* Enemy intel stats */}
              <div className="flex gap-0.5 justify-center mt-0.5 flex-wrap">
                {state.enemy_intel.radar_known && (
                  <span className="text-[7px] text-violet-400 bg-violet-500/10 rounded px-1">R:{state.enemy_intel.radar_range_km}km</span>
                )}
                {state.enemy_intel.rcs_known && (
                  <span className="text-[7px] text-violet-400 bg-violet-500/10 rounded px-1">{state.enemy_intel.rcs_m2}m²</span>
                )}
                {state.enemy_intel.ecm_known && (
                  <span className="text-[7px] text-violet-400 bg-violet-500/10 rounded px-1">ECM:{state.enemy_intel.ecm_rating}</span>
                )}
                {state.enemy_intel.damage_known && (
                  <span className="text-[7px] text-red-400 bg-red-500/10 rounded px-1">-{state.enemy_intel.damage_pct?.toFixed(0)}%</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {state.player_ammo.map((a, i) => (
              <div key={i} className="flex items-center gap-1 bg-gray-900/60 rounded px-1.5 py-0.5">
                ...
                <span className="text-[7px] text-gray-400 hud-text">{a.weapon_name.split(' ').pop()}</span>
                <span className="text-[8px] font-bold text-emerald-400 hud-text">{a.remaining}</span>
              </div>
            ))}
            {state.ecm_charges > 0 && (
              <div className="flex items-center gap-1 bg-gray-900/60 rounded px-1.5 py-0.5">
                <Radio className="w-2.5 h-2.5 text-cyan-400/60" />
                <span className="text-[7px] text-gray-400 hud-text">ECM</span>
                <span className="text-[8px] font-bold text-cyan-400 hud-text">{state.ecm_charges}</span>
              </div>
            )}
            {state.flare_uses > 0 && (
              <div className="flex items-center gap-1 bg-gray-900/60 rounded px-1.5 py-0.5">
                <Zap className="w-2.5 h-2.5 text-amber-400/60" />
                <span className="text-[7px] text-gray-400 hud-text">FLR</span>
                <span className="text-[8px] font-bold text-amber-400 hud-text">{state.flare_uses}</span>
              </div>
            )}
            <div className="flex items-center gap-1 bg-gray-900/60 rounded px-1.5 py-0.5">
              <Eye className="w-2.5 h-2.5 text-violet-400/60" />
              <span className="text-[7px] text-gray-400 hud-text">INTEL</span>
              <span className="text-[8px] font-bold text-violet-400 hud-text">{intelCount}/6</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 px-3 py-2 flex flex-col min-h-0 overflow-hidden">
        {error && !showingResult && (
          <div className="mb-2 rounded-xl bg-red-500/10 border border-red-500/30 p-2.5 flex items-center gap-2">
            ...
            <button onClick={() => setError(null)} className="text-xs text-red-400 bg-red-500/20 px-2 py-1 rounded">
        ...
        {showingResult && turnResult ? (
          <div className="flex-1 flex flex-col phase-slide-in overflow-y-auto">
            ...
            <div className="rounded-xl bg-gray-900/60 p-3 mb-2">
              ...
              {/* Your action + enemy action */}
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div className="bg-emerald-500/5 rounded-lg p-2">
                  <p className="text-[8px] text-emerald-400/60 hud-text">YOU</p>
                ...
                <div className="bg-red-500/5 rounded-lg p-2">
                  <p className="text-[8px] text-red-400/60 hud-text">ENEMY</p>
              ...
              {turnResult.shot_hit !== undefined && turnResult.shot_hit !== null && (
                <div className={`rounded-lg p-2 mb-1.5 text-center ${turnResult.shot_hit ? 'bg-emerald-500/10 hud-border' : 'bg-red-500/10 hud-border-red'}`}>
                  ...
                  <p className="text-[10px] text-gray-400 hud-text">
              ...
              {turnResult.enemy_shot_hit !== undefined && turnResult.enemy_shot_hit !== null && (
                <div className={`rounded-lg p-2 mb-1.5 text-center ${turnResult.enemy_shot_hit ? 'bg-red-500/10 hud-border-red' : 'bg-emerald-500/10 hud-border'}`}>
                  ...
                  <p className="text-[10px] text-gray-500 hud-text">{turnResult.enemy_weapon_fired}</p>
                </div>
              )}
              ...
              {state.available_actions.map((action) => {
                ...
                return (
                  <button
                    ...
                    disabled={choosing}
                    className={`w-full rounded-xl p-2.5 text-left transition-all active:scale-[0.98] disabled:opacity-40 bg-gray-950/80 ${
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isFire && action.pk_preview != null && (
                            <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border hud-text ${pkBg(action.pk_preview)} ${pkColor(action.pk_preview)}`}>
                              Pk {(action.pk_preview * 100).toFixed(0)}%
                            </span>
                          )}
                        </div>
                        <p className="text-[10px] text-gray-500 mt-0.5 hud-text">{action.description}</p>
                      </div>
