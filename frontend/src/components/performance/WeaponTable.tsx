...
function Row({ w }: { w: WeaponStat }) {
  ...
  return (
    <tr className="border-b border-slate-900/60">
      ...
      <td className="hidden sm:table-cell py-1 px-2 text-right font-mono">{w.avg_pk.toFixed(2)}</td>
      <td className="hidden sm:table-cell py-1 px-2 text-right font-mono">
      ...
      <td className="hidden sm:table-cell py-1 pl-2 font-mono opacity-80">{w.top_target_platform ?? "—"}</td>
    </tr>
  );
}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-left opacity-60 border-b border-slate-800">
              ...
              <th className="hidden sm:table-cell py-1 px-2 font-medium text-right">Avg PK</th>
              <th className="hidden sm:table-cell py-1 px-2 font-medium text-right">Total ₹</th>
              <th className="py-1 px-2 font-medium text-right">₹ / Kill</th>
              <th className="hidden sm:table-cell py-1 pl-2 font-medium">Top target</th>
            </tr>
          </thead>
